...
(also with the same color as the starting pixel), and so on. Replace the color of all of the aforementioned pixels with 
the newColor.
...
private:
    void flood_fill(vector<vector<int>>& image, int sr, int sc, int newColor){
        ...
        if((sr - 1) >= 0 && image[sr - 1][sc] == old)
           flood_fill(image, sr - 1, sc, newColor);
    
        if((sc + 1) < image[0].size() && image[sr][sc + 1] == old)
            flood_fill(image, sr, sc + 1, newColor);
